...
var loadScript = function loadScript(scriptName, callback) {
  if (!jsArray[scriptName]) {
    var promise = jQuery.Deferred(); // adding the script tag to the head as suggested before

    var body = document.getElementsByTagName('body')[0],
        script = document.createElement('script');
    script.type = 'text/javascript';
    script.src = scriptName; // then bind the event to the callback function
    ...
    body.appendChild(script); // clear DOM reference
    //body = null;
    //script = null;
...
$(document).on('click', '#js-nav-menu a[href!="#"]', function (e) {
  e.preventDefault();
  var $this = $(e.currentTarget); // if parent is not active then get hash, or else page is assumed to be loaded
  ...
  if (!$this.parent().hasClass("active") && !$this.attr('target')) {
    ...
    if (myapp_config.root_.hasClass('mobile-view-activated')) {
      ...
      }, 150); // it may not need this delay...
    } else {
      if (window.location.search) {
        window.location.href = window.location.href.replace(window.location.search, '').replace(window.location.hash, '') + '#' + $this.attr('href');
      } else {
...
function checkURL() {
  ...
  var url = location.href.split('#').splice(1).join('#'); //BEGIN: IE11 Work Around
  ...
  if (url) {
    ...
    $('#js-nav-menu li.active').removeClass("active"); // match the url and add the active class

    $('#js-nav-menu li:has(a[href="' + url + '"])').addClass("active");
    var title = $('nav a[href="' + url + '"]').attr('title'); // change page title from global var

    document.title = title || document.title; // debugState

    console.log("Page title: %c " + document.title); // parse url to jquery

    loadURL(url + location.search, container);
  } else {
    // grab the first URL from nav
    var $this = $('#js-nav-menu > li:first-child > a[href!="#"]'); //update hash

    window.location.hash = $this.attr('href'); //clear dom reference
...
function loadURL(url, container) {
  ...
  $.ajax({
    ...
    beforeSend: function beforeSend() {
      ...
      pagefunction = null;
      container.removeData().html(""); // place cog

      container.html('<h1 class="ajax-loading-animation"><i class="fa fa-cog fa-spin"></i> Loading...</h1>'); // Only draw breadcrumb if it is main content material
      ...
      if (container[0] == $("#js-page-content")[0]) {
        ...
        $('body').find('> *').filter(':not(' + ignore_key_elms + ')').empty().remove(); // draw breadcrumb
        //drawBreadCrumb();
        // scroll up
...
function pageSetUp() {
  if (thisDevice === "desktop") {
    // is desktop
    // activate tooltips
    $("[rel=tooltip], [data-rel=tooltip]").tooltip(); // activate popovers

    $("[rel=popover], [data-rel=popover]").popover(); // activate popovers with hover states

    $("[rel=popover-hover], [data-rel=popover-hover]").popover({
      trigger: "hover"
    }); // setup widgets

    setup_widgets_desktop(); // activate inline charts

    runAllCharts(); // run form elements
    ...
    runAllCharts(); // setup widgets

    setup_widgets_mobile(); // run form elements
