...
var initApp = function (app) {
  ...
  app.listFilter = function (list, input, anchor) {
    ...
    $(input).change(function () {
      ...
      if (filter.length > 1) {
        ...
        $(list).find($("[data-filter-tags]:not([data-filter-tags*='" + filter + "'])")).parentsUntil(list).removeClass('js-filter-show').addClass('js-filter-hide');
        /* (2) hide all that does match */

        $(list).find($("[data-filter-tags*='" + filter + "']")).parentsUntil(list).removeClass('js-filter-hide').addClass('js-filter-show');
  ...
  app.factoryReset = function () {
    //backdrop sound
    initApp.playSound('media/sound', 'messagebox'); //hide settings modal to bootstrap avoid modal bug
  ...
  app.playSound = function (path, sound) {
    ...
    if (navigator.userAgent.match('Firefox/')) audioElement.setAttribute('src', path + "/" + sound + '.ogg');else audioElement.setAttribute('src', path + "/" + sound + '.mp3'); //$.get();// <-- ??
    ...
    myapp_config.root_.on('click touchend', '[data-action]', function (e) {
      ...
      switch (true) {
        ...
        case actiontype === 'toggle-replace':
          ...
          $(target).removeClass(replaceClass).addClass(dataClass); //console.log( replaceClass + " :replace these classes")
        ...
        case actiontype === 'panel-collapse':
          ...
          selectedPanel.children('.panel-container').collapse('toggle').on('show.bs.collapse', function () {
        ...
        case actiontype === 'lang':
          ...
          if (!$.i18n) {
            ...
            initApp.loadScript("js/i18n/i18n.js", function activateLang() {
              $.i18n.init({
