...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
     ...
     * @param:
     **/
    _runPanelLoader: function _runPanelLoader(elm) {
      var self = this;

      if (self.o.localStorage === true) {
        elm.closest(self.o.panels).find('.panel-saving').stop(true, true).fadeIn(100).delay(600).fadeOut(100);
      }
    },
     ...
     * @param:
     **/
    _savePanelSettings: function _savePanelSettings() {
      var self = this;
      var storage = self.storage;

      self._loadKeys();

      var storeSettings = self.obj.find(self.o.panels).map(function () {
      ...
      if (storage.enabled && storage.getKeySettings != storeSettingsObj) {
        ...
        storage.getKeySettings = storeSettingsObj; //if (myapp_config.debugState)
      ...
      if (typeof self.o.onSave == 'function') {
        ...
        if (myapp_config.debugState) console.log("keySettings: " + storage.keySettings);
      }
    },
     ...
     * @param:
     **/
    _savePanelPosition: function _savePanelPosition() {
      var self = this;
      var storage = self.storage;
      ...
      var mainArr = self.obj.find(self.o.grid + '.sortable-grid').map(function () {
        var subArr = $(this).children(self.o.panels).map(function () {
          return {
            'id': $(this).attr('id')
          };
        }).get();
    ...
    init: function init() {
      ...
      if (self.storage.enabled && self.storage.getKeySettings) {
        ...
        if (myapp_config.debugState) console.log("Panel settings loaded: " + self.storage.getKeySettings);
        /**
        ...
        for (var key in jsonSettings.panel) {
          ...
          if (jsonSettings.panel[key].style) {
            panelId.attr('data-panel-attstyle', '' + jsonSettings.panel[key].style + '').children('.panel-hdr').removeClassPrefix('bg-').addClass(jsonSettings.panel[key].style);
          }
          /**
          ...
          if (jsonSettings.panel[key].collapsed == 1) {
            panelId.addClass('panel-collapsed').children('.panel-container').addClass('collapse').removeClass('show');
          }
          /**
        ...
        if (!thisHeader.parent().attr('role')) {
          ...
          **/
          ...
          if (self.o.refreshButton === true && tPanel.data('panel-refresh') === undefined) {
            ...
            thisContainer.prepend('<div class="loader"><i class="fal fa-spinner-third fa-spin-4x fs-xxl"></i></div>'); //append** conflicts with panel > container > content:last child, so changed to prepend
          } else {
            refreshButton = '';
          }
          /**
          ...
          **/
          ...
          thisHeader.append('<div class="panel-saving mr-2" style="display:none"><i class="fal fa-spinner-third fa-spin-4x fs-xl"></i></div>');
          /**
           * Set the buttons order.
           **/

          var formatButtons = self.o.buttonOrder.replace(/%close%/g, closeButton).replace(/%fullscreen%/g, fullscreenButton).replace(/%collapse%/g, collapseButton);
          /**
          ...
          var formatDropdownButtons = self.o.buttonOrderDropdown.replace(/%locked%/g, lockedButton).replace(/%color%/g, colorButton).replace(/%refresh%/g, refreshButton).replace(/%reset%/g, resetButton).replace(/%custom%/g, customButton);
          /**
           * Add a button wrapper to the header.
           **/
          ...
          tPanel.attr('role', 'widget').children('div').attr('role', 'content').prev('.panel-hdr').attr('role', 'heading').children('.panel-toolbar').attr('role', 'menu');
        }
      });
      /**
       * SORTABLE
      ...
      if (self.o.sortable === true && jQuery.ui) {
        var sortItem = self.obj.find(self.o.grid + '.sortable-grid').not('[data-panel-excludegrid]');
        sortItem.sortable({
    ...
    _clickEvents: function _clickEvents() {
      ...
      headers.on(clickEvent, '.js-panel-collapse', function (e) {
        ...
        pPanel.children('.panel-container').collapse('toggle').on('shown.bs.collapse', function () {
      ...
      headers.on(clickEvent, '.js-panel-close', function (e) {
        ...
        var killPanel = function killPanel() {
          /**
           * Run function for the indicator image.
           **/
      ...
      headers.on(clickEvent, '.js-panel-color', function (e) {
        ...
        selectedHdr.removeClassPrefix('bg-').addClass(val).closest('.panel').attr('data-panel-attstyle', '' + val + '');
        /**
         * Run the callback function.
         **/
      ...
      headers.on(clickEvent, '.js-panel-refresh', function (e) {
        ...
        pPanel.addClass('panel-refresh').children('.panel-container').addClass('enable-loader').stop(true, true).delay(dTimer).queue(function () {
      ...
      **/
      ...
      headers.on(clickEvent, '.js-panel-reset', function (e) {
        ...
        selectedHdr.removeClassPrefix('bg-').closest('.panel').removeClass('panel-collapsed panel-fullscreen panel-locked').attr('data-panel-attstyle', '').children('.panel-container').collapse('show');
        /**
         * Run function for the indicator image.
         **/
