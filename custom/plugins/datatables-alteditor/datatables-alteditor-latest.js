...
(function (factory) {
  ...
  var altEditor = function altEditor(dt, opts) {
  ...
  $.extend(altEditor.prototype, {
    ...
    _constructor: function _constructor() {
      var that = this;
      var dt = this.s.dt;
      if (dt.settings()[0].oInit.onAddRow) that.onAddRow = dt.settings()[0].oInit.onAddRow;
      if (dt.settings()[0].oInit.onDeleteRow) that.onDeleteRow = dt.settings()[0].oInit.onDeleteRow;
      if (dt.settings()[0].oInit.onEditRow) that.onEditRow = dt.settings()[0].oInit.onEditRow;
    ...
    _setup: function _setup() {
      ...
      var modal_id = "altEditor-modal-".concat(this.random_id);
      ...
      var modal = '<div class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog">' + '<div class="modal-dialog modal-lg">' + '<div class="modal-content">' + '<div class="modal-header">' + '<h5 class="modal-title"></h5>' + '<button type="button" class="close" data-dismiss="modal" aria-label="' + this.language.modalClose + '">' + '<span aria-hidden="true"><i class="fal fa-times"></i></span></button>' + '</div>' + '<div class="modal-body">' + '<p></p>' + '</div>' + '<div class="modal-footer">' + '</div>' + '</div>' + '</div>' + '</div>'; // add modal

      $('body').append(modal); // add Edit Button

      if (dt.button('edit:name')) {
        dt.button('edit:name').action(function (e, dt, node, config) {
          that._openEditModal();

          $("#altEditor-edit-form-".concat(that.random_id)).off('submit').on('submit', function (e) {
            e.preventDefault();
            e.stopPropagation();
      ...
      if (dt.button('delete:name')) {
        dt.button('delete:name').action(function (e, dt, node, config) {
          ...
          $("#altEditor-delete-form-".concat(that.random_id)).off('submit').on('submit', function (e) {
            e.preventDefault();
            e.stopPropagation();
      ...
      if (dt.button('add:name')) {
        dt.button('add:name').action(function (e, dt, node, config) {
          ...
          $("#altEditor-add-form-".concat(that.random_id)).off('submit').on('submit', function (e) {
            e.preventDefault();
            e.stopPropagation();
      ...
      $(this.modal_selector).bind('input', '[data-unique]', function (elm) {
        ...
        var colData = dt.column("th:contains('" + target.attr("name") + "')").data(); // go through each item in this column

        var selectedCellData = null;
        if (dt.row({
          selected: true
        }).index() != null) selectedCellData = dt.cell(dt.row({
          selected: true
        }).index(), dt.column("th:contains('" + target.attr("name") + "')").index()).data();
    ...
    _openEditModal: function _openEditModal() {
      ...
      var data = this.createDialog(columnDefs, this.language.edit.title, this.language.edit.button, this.language.modalClose, 'editRowBtn', 'altEditor-edit-form');
     ...
     */
    _editRowData: function _editRowData() {
      var that = this;
      var dt = this.s.dt; // Complete new row data
      ...
      $("form[name=\"altEditor-edit-form-".concat(this.random_id, "\"] *")).filter(':input[type!="file"]').each(function (i) {
        rowDataArray[$(this).attr('id')] = $(this).val();
      }); //Getting the textArea from the modal

      $("form[name=\"altEditor-edit-form-".concat(this.random_id, "\"] *")).filter('textarea').each(function (i) {
        rowDataArray[$(this).attr('id')] = $(this).val();
      ...
      var checkFilesQueued = function checkFilesQueued() {
        if (numFilesQueued == 0) {
     ...
     * @private
     */
    _openDeleteModal: function _openDeleteModal() {
      var that = this;
      var dt = this.s.dt;
      var adata = dt.rows({
        selected: true
      ...
      var columnDefs = this.completeColumnDefs();
      var formName = 'altEditor-delete-form-' + this.random_id; // TODO
      ...
      for (var j in columnDefs) {
        if (columnDefs[j].type.indexOf("hidden") >= 0) {
          ...
          data += "<div class='row mb-3 mb-sm-4'><label class='col-lg-3 col-form-label form-label text-left text-lg-right' for='" + that._quoteattr(columnDefs[j].name) + "'>" + columnDefs[j].title + ":</label> <div class='col-lg-9 d-flex align-items-center'><div class='border rounded w-100 px-3 py-2 bg-faded'><input type='hidden' id='" + that._quoteattr(columnDefs[j].title) + "' name='" + that._quoteattr(columnDefs[j].title) + "' placeholder='" + that._quoteattr(columnDefs[j].placeholderMsg) + "' style='overflow:hidden'  class='form-control' value='" + that._quoteattr(adata.data()[0][columnDefs[j].name]) + "' >" + adata.data()[0][columnDefs[j].name] + "</input></div></div></div></div>";
        }
      }

      var selector = this.modal_selector;
      $(selector).on('show.bs.modal', function () {
        var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">' + that.language.modalClose + '</button>' + '<button type="submit"  data-content="remove" class="btn btn-danger" id="deleteRowBtn"><i class="fal fa-times mr-1"></i>  ' + that.language.delete.button + '</button>';
    ...
    _deleteRow: function _deleteRow() {
      ...
      for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
        ...
        var name = dt.context[0].aoColumns[i].data ? dt.context[0].aoColumns[i].data : dt.context[0].aoColumns[i].mData ? dt.context[0].aoColumns[i].mData : dt.context[0].aoColumns[i].idx;
        jsonDataArray[name] = adata.data()[0][name];
      }

      that.onDeleteRow(that, jsonDataArray, function (data) {
     ...
     * @private
     */
    _openAddModal: function _openAddModal() {
      var dt = this.s.dt;
      var columnDefs = this.completeColumnDefs();
    ...
    */
    completeColumnDefs: function completeColumnDefs() {
      var columnDefs = [];
      var dt = this.s.dt;
    ...
    */
    createDialog: function createDialog(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
      ...
      for (var j in columnDefs) {
        ...
        if (columnDefs[j].type.indexOf("hidden") >= 0) {
          ...
          data += "<div class='col-lg-9'>"; // Adding readonly-fields

          if (columnDefs[j].type.indexOf("readonly") >= 0) {
            // type=readonly is deprecated, kept for backward compatibility
            data += "<input type='text' readonly  id='" + this._quoteattr(columnDefs[j].name) + "' name='" + this._quoteattr(columnDefs[j].title) + "' placeholder='" + this._quoteattr(columnDefs[j].placeholderMsg) + "' style='overflow:hidden'  class='form-control' value=''>";
              ...
              if (optionsArray.length > 0) {
                ...
                for (var i = 0; i < optionsArray.length; i++) {
                  options += "<option value='" + this._quoteattr(optionsArray[i]) + "'>" + optionsArray[i] + "</option>";
                }
              } else {
                // object-style select or select2
                for (var x in optionsArray) {
                  options += "<option value='" + this._quoteattr(x) + "' >" + optionsArray[x] + "</option>";
                }
              }

              data += "<select class='form-control" + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + this._quoteattr(columnDefs[j].name) + "' name='" + this._quoteattr(columnDefs[j].title) + "' placeholder='" + this._quoteattr(columnDefs[j].placeholderMsg) + "' data-special='" + this._quoteattr(columnDefs[j].special) + "' data-errorMsg='" + this._quoteattr(columnDefs[j].msg) + "' data-uniqueMsg='" + this._quoteattr(columnDefs[j].uniqueMsg) + "' data-unique='" + columnDefs[j].unique + "' " + (columnDefs[j].readonly ? ' readonly ' : '') + (columnDefs[j].disabled ? ' disabled ' : '') + (columnDefs[j].required ? ' required ' : '') + (columnDefs[j].multiple ? ' multiple ' : '') + ">" + options + "</select>";
            } //Adding Text Area 
            else if (columnDefs[j].type.indexOf("textarea") >= 0) {
                data += "<textarea id='" + this._quoteattr(columnDefs[j].name) + "' name='" + this._quoteattr(columnDefs[j].title) + "' rows='" + this._quoteattr(columnDefs[j].rows) + "' cols='" + this._quoteattr(columnDefs[j].cols) + "' placeholder='" + this._quoteattr(columnDefs[j].placeholderMsg) + "' data-special='" + this._quoteattr(columnDefs[j].special) + "' data-errorMsg='" + this._quoteattr(columnDefs[j].msg) + "' data-uniqueMsg='" + this._quoteattr(columnDefs[j].uniqueMsg) + "' data-unique='" + columnDefs[j].unique + "' " + (columnDefs[j].readonly ? ' readonly ' : '') + (columnDefs[j].disabled ? ' disabled ' : '') + (columnDefs[j].required ? ' required ' : '') + (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'") + ">" + "</textarea>";
              } // Adding text-inputs and errorlabels, but also new HTML5 typees (email, color, ...)
              else {
                  data += "<input type='" + this._quoteattr(columnDefs[j].type) + "' id='" + this._quoteattr(columnDefs[j].name) + "' pattern='" + this._quoteattr(columnDefs[j].pattern) + "' title='" + this._quoteattr(columnDefs[j].hoverMsg) + "' name='" + this._quoteattr(columnDefs[j].title) + "' placeholder='" + this._quoteattr(columnDefs[j].placeholderMsg) + "' data-special='" + this._quoteattr(columnDefs[j].special) + "' data-errorMsg='" + this._quoteattr(columnDefs[j].msg) + "' data-uniqueMsg='" + this._quoteattr(columnDefs[j].uniqueMsg) + "' data-unique='" + columnDefs[j].unique + "' " + (columnDefs[j].readonly ? ' readonly ' : '') + (columnDefs[j].disabled ? ' disabled ' : '') + (columnDefs[j].required ? ' required ' : '') + (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'") + " style='overflow:hidden'  class='form-control' value=''>";
                }

          data += "<span id='" + this._quoteattr(columnDefs[j].name) + "label" + "' class='errorLabel help-block text-danger'></span>";
      ...
      var selector = this.modal_selector;
      $(selector).on('show.bs.modal', function () {
        var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">' + closeCaption + '</button>' + '<button type="submit" form="' + formName + '" data-content="remove" class="btn btn-primary" id="' + buttonClass + '">' + buttonCaption + '</button>';
      ...
      $(selector).modal('show');
      $(selector + ' input[0]').focus();
      var that = this; // enable select 2 items, datepicker, datetimepickerm
     ...
     */
    _addRowData: function _addRowData() {
      var that = this;
      var dt = this.s.dt;
      var rowDataArray = {}; // Getting the inputs from the modal

      $("form[name=\"altEditor-add-form-".concat(this.random_id, "\"] *")).filter(':input[type!="file"]').each(function (i) {
        rowDataArray[$(this).attr('id')] = $(this).val();
      }); //Getting the textArea from the modal

      $("form[name=\"altEditor-add-form-".concat(this.random_id, "\"] *")).filter('textarea').each(function (i) {
        rowDataArray[$(this).attr('id')] = $(this).val();
      }); //Getting Files from the modal
      ...
      var checkFilesQueued = function checkFilesQueued() {
        if (numFilesQueued == 0) {
          that.onAddRow(that, rowDataArray, function (data) {
     ...
     */
    _deleteRowCallback: function _deleteRowCallback(response, status, more) {
      var selector = this.modal_selector;
      $(selector + ' .modal-body .alert').remove();
      var message = '<div class="alert alert-success" role="alert">' + '<strong>' + this.language.success + '</strong>' + '</div>';
      $(selector + ' .modal-body').prepend(message);
      this.s.dt.row({
        selected: true
      }).remove();
      this.s.dt.draw('page'); // Disabling submit button

      $("div" + selector).find("button#addRowBtn").prop('disabled', true);
      $("div" + selector).find("button#editRowBtn").prop('disabled', true);
      $("div" + selector).find("button#deleteRowBtn").prop('disabled', true); //Hide modal
     ...
     */
    _addRowCallback: function _addRowCallback(response, status, more) {
      ...
      var message = '<div class="alert alert-success" role="alert">' + '<strong>' + this.language.success + '</strong>' + '</div>';
      $(selector + ' .modal-body').prepend(message);
      this.s.dt.row.add(data).draw(false); // Disabling submit button
     ...
     */
    _editRowCallback: function _editRowCallback(response, status, more) {
      //TODO should honor dt.ajax().dataSrc
      var data = typeof response === "string" ? JSON.parse(response) : response;
      var selector = this.modal_selector;
      $(selector + ' .modal-body .alert').remove();
     ...
     */
    _errorCallback: function _errorCallback(response, status, more) {
      var error = response;
      var selector = this.modal_selector;
      $(selector + ' .modal-body .alert').remove();
      ...
      var message = '<div class="alert alert-danger" role="alert">' + '<strong>' + this.language.error.label + '</strong> ' + (error.status == null ? "" : this.language.error.responseCode + error.status) + " " + errstr + '</div>';
      $(selector + ' .modal-body').prepend(message);
    },
     ...
     */
    onAddRow: function onAddRow(dt, rowdata, success, error) {
     ...
     */
    onEditRow: function onEditRow(dt, rowdata, success, error) {
     ...
     */
    onDeleteRow: function onDeleteRow(dt, rowdata, success, error) {
    ...
    */
    reloadOptions: function reloadOptions($select, options) {
     ...
     */
    getBase64: function getBase64(file, onSuccess, onError) {
     ...
     * @private
     */
    _quoteattr: function _quoteattr(s, preserveCR) {
      if (s == null) return "";
  ...
  $(document).on('preInit.dt.altEditor', function (e, settings, json) {
    ...
    if (init || defaults) {
      ...
      if (init !== false) {
        var editor = new altEditor(settings, opts); // e is a jQuery event object
