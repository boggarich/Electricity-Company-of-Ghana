...
(function (factory) {
  ...
  $.extend(altEditor.prototype, {
    ...
    _setup: function _setup() {
      ...
      var modal = '<div class="modal fade" id="' + modal_id + '" tabindex="-1" role="dialog">' + '<div class="modal-dialog">' + '<div class="modal-content">' + '<div class="modal-header">' + '<h5 class="modal-title"></h5>' + '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fal fa-times"></i></span></button>' + '</div>' + '<div class="modal-body">' + '<p></p>' + '</div>' + '<div class="modal-footer">' + '<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>' + //FIXME need i18n
      '<input type="submit" form="altEditor-form" class="btn btn-primary"></input>' + '</div>' + '</div>' + '</div>' + '</div>'; // add modal
    ...
    _openEditModal: function _openEditModal() {
      var that = this;
      var dt = this.s.dt;
      var columnDefs = []; // Adding column attributes to object.
      ...
      for (var j in columnDefs) {
        ...
        if (columnDefs[j].type.indexOf("hidden") >= 0) {
          ...
          data += "<div class='col-lg-9'>"; // Adding text-inputs and errorlabels

          if (columnDefs[j].type.indexOf("text") >= 0 || columnDefs[j].type.indexOf("date") >= 0 || columnDefs[j].type.indexOf("email") >= 0 || columnDefs[j].type.indexOf("password") >= 0 || columnDefs[j].type.indexOf("number") >= 0 || columnDefs[j].type.indexOf("time") >= 0) {
            data += "<input type='" + that._quoteattr(columnDefs[j].type) + "' id='" + that._quoteattr(columnDefs[j].name) + "'  pattern='" + that._quoteattr(columnDefs[j].pattern) + "'  title='" + that._quoteattr(columnDefs[j].hoverMsg) + "' name='" + that._quoteattr(columnDefs[j].title) + "' placeholder='" + that._quoteattr(columnDefs[j].placeholderMsg) + "' data-special='" + that._quoteattr(columnDefs[j].special) + "' data-errorMsg='" + that._quoteattr(columnDefs[j].msg) + "' data-uniqueMsg='" + that._quoteattr(columnDefs[j].uniqueMsg) + "' data-unique='" + columnDefs[j].unique + "'" + (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'") + " style='overflow:hidden' class='form-control' value='" + that._quoteattr(adata.data()[0][columnDefs[j].name]) + "'>";
            data += "<label id='" + columnDefs[j].name + "label" + "' class='errorLabel help-block text-danger' style='display: none'></label>";
          } // Adding readonly-fields


          if (columnDefs[j].type.indexOf("readonly") >= 0) {
            data += "<input type='text' readonly  id='" + columnDefs[j].name + "' name='" + columnDefs[j].title + "' placeholder='" + columnDefs[j].placeholderMsg + "' style='overflow:hidden' class='form-control' value='" + adata.data()[0][columnDefs[j].name] + "'>";
          ...
          if (columnDefs[j].type.indexOf("select") >= 0) {
            ...
            for (var i = 0; i < columnDefs[j].options.length; i++) {
              ...
              if (adata.data()[0][columnDefs[j].name].toString().indexOf(columnDefs[j].options[i]) >= 0) {
            ...
            data += "<select class='custom-select form-control" + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + columnDefs[j].name + "' name='" + columnDefs[j].title + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options + "</select>";
          }
      ...
      $(selector).on('show.bs.modal', function () {
        var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cancel</button>' + '<button type="button" data-content="remove" class="btn btn-primary" id="editRowBtn">Save</button>';
        $(selector).find('.modal-title').html('Edit Record');
        $(selector).find('.modal-body').html(data);
        $(selector).find('.modal-footer').html(btns);
      });
      $(selector).modal('show');
      $(selector + ' input[0]').focus(); // enable select 2 items
    ...
    _openDeleteModal: function _openDeleteModal() {
      var that = this;
      var dt = this.s.dt;
      var columnDefs = []; // Adding attribute IDs and values to object
      ...
      var selector = this.modal_selector;
      $(selector).on('show.bs.modal', function () {
        var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cancel</button>' + '<button type="button"  data-content="remove" class="btn btn-danger" id="deleteRowBtn"><i class="fal fa-times"></i> Delete</button>';
    ...
    _openAddModal: function _openAddModal() {
      ...
      for (var j in columnDefs) {
        if (columnDefs[j].type.indexOf("hidden") >= 0) {// just do nothing for hidden fields!
        } else {
          data += "<div class='form-group row'>" + "<label class='form-label col-sm-3 col-form-label text-left text-sm-right' for='" + columnDefs[j].name + "'>" + columnDefs[j].title + ":</label><div class='col-lg-9'>"; // Adding text-inputs and errorlabels

          if (columnDefs[j].type.indexOf("text") >= 0 || columnDefs[j].type.indexOf("date") >= 0 || columnDefs[j].type.indexOf("email") >= 0 || columnDefs[j].type.indexOf("password") >= 0 || columnDefs[j].type.indexOf("number") >= 0 || columnDefs[j].type.indexOf("time") >= 0) {
            data += "<input type='" + that._quoteattr(columnDefs[j].type) + "' id='" + that._quoteattr(columnDefs[j].name) + "'  pattern='" + that._quoteattr(columnDefs[j].pattern) + "'  title='" + that._quoteattr(columnDefs[j].hoverMsg) + "' name='" + that._quoteattr(columnDefs[j].title) + "' placeholder='" + that._quoteattr(columnDefs[j].placeholderMsg) + "' value='" + that._quoteattr(columnDefs[j].defaultValue) + "' data-special='" + columnDefs[j].special + "' data-errorMsg='" + that._quoteattr(columnDefs[j].msg) + "' data-uniqueMsg='" + that._quoteattr(columnDefs[j].uniqueMsg) + "' data-unique='" + columnDefs[j].unique + "'" + (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'") + " style='overflow:hidden' class='form-control' value=''>";
            data += "<label id='" + that._quoteattr(columnDefs[j].name) + "label" + "' class='errorLabel help-block text-danger' style='display:none'></label>";
          } // Adding readonly-fields


          if (columnDefs[j].type.indexOf("readonly") >= 0) {
            data += "<input type='text' readonly  id='" + that._quoteattr(columnDefs[j].name) + "' name='" + that._quoteattr(columnDefs[j].title) + "' placeholder='" + that._quoteattr(columnDefs[j].placeholderMsg) + "' style='overflow:hidden' class='form-control' value=''>";
          } // Adding select-fields
          ...
          if (columnDefs[j].type.indexOf("select") >= 0) {
            ...
            for (var i = 0; i < columnDefs[j].options.length; i++) {
              options += "<option value='" + that._quoteattr(columnDefs[j].options[i]) + "'>" + columnDefs[j].options[i] + "</option>";
            }

            data += "<select class='custom-select form-control" + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + that._quoteattr(columnDefs[j].name) + "' name='" + that._quoteattr(columnDefs[j].title) + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options + "</select>";
          }
      ...
      var selector = this.modal_selector;
      $(selector).on('show.bs.modal', function () {
        var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cancel</button>' + '<button type="button"  data-content="remove" class="btn btn-primary" id="addRowBtn">Add</button>';
    ...
    _deleteRowCallback: function _deleteRowCallback(response, status, more) {
      ...
      var message = '<div class="alert alert-success" role="alert">' + '<strong>Success!</strong>' + '</div>';
      $(selector + ' .modal-body').append(message);
      this.s.dt.row({
        selected: true
      }).remove();
      this.s.dt.draw(); // Disabling submit button
    ...
    _errorCallback: function _errorCallback(response, status, more) {
      ...
      var message = '<div class="alert alert-danger" role="alert">' + '<strong>Error!</strong> ' + (error.status == null ? "" : 'Response code: ' + error.status) + " " + errstr + '</div>';
      $(selector + ' .modal-body').append(message);
    },
     ...
     * @private
     */
    _inputValidation: function _inputValidation() {
      var that = this;
      var dt = this.s.dt;
      var isValid = false;
      var errorcount = 0; // Looping through all text fields

      $('form[name="altEditor-form"] *').filter(':text').each(function (i) {
        var errorLabel = "#" + $(this).attr("id") + "label"; // reset error display
        ...
        if (!$(this).val().match($(this).attr("pattern"))) {
          ...
          errorcount++;
            ...
            if (dt.row({
              ...
              selected: true
            }).index(), dt.column("th:contains('" + $(this).attr("name") + "')").index()).data();
